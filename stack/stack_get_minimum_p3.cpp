 ...
 * push the difference of [2 * current element(big) and currentMin(small)] 
 * in the stack and make the Minimum Element equal to the current element.
 * 
 * This might result in the pushing of a -ve value in the stack which will 
 ...
 * Here we are multiplying the current value by 2, so that we can handle
 * incoming negative values as well.
 * 
 ...
 * by adding the popped element with the TWICE OF minimum Element and finally pop()
